...
public class MapperTest2 {
	...
	public void convert_test2() {
		...
		// List<Dog> -> List<DogDto>
		List<DogDto> dogDtoList = dogList.stream() // stream: DogDto를 감싸고 있는 최상위 타입(list)를 벗김. 더이상 list가 아니게 됨
				.map((dog) -> new DogDto(dog)) // .: forEach map: 타입을 변화시킴 (Dog -> DogDto)
				.collect(Collectors.toList()); // stream에 있는것을 다시 list에 넣음
